...
ShaderBase::ShaderBase(const ShaderBase& shader) : m_counter(shader.m_counter)
{
	m_id = shader.m_id;
	(*m_counter)++;
}

ShaderBase& ShaderBase::operator=(const ShaderBase& shader)
{
	m_id = shader.m_id;
	(*m_counter)++;
	return *this;
...
Shader Shader::from_file(const char* vertex_file,
